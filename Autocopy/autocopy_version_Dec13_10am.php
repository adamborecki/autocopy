...
while(!$ready){
    ...
    if($check_num_volumes != $num_volumes){
        echo "FATAL ERROR!  The number of volumes has changed. Please restart autocopy.";
        echo "\n";
...
echo "\t($script_info[name] does not wait and currently does NOT validate if the data was copied successfully!)\n\n\n";
        ...
        foreach($volumes as $index => $volume){
            if($purpose_icon == $volume['purpose']){
                ...
                if($volume['purpose']==PURPOSE_DEST){
                    ...
                    echo "        ";
                    if($enough_space)
                        echo "√ Has enough free space";
                    else
                        echo "X DOES NOT HAVE ENOUGH FREE SPACE";
                    echo "\n";
                }
                if(!$config['has_displayed_once'])
                    passthru("sleep 0.1");
...
function confirm_num_sources($volumes){
    global $script_info;
    $num_sources = 0;
    foreach($volumes as $volume)
        if($volume['purpose'] == PURPOSE_SRC)
